...
class SwarmEnv(gym.Env):
    ...
    def step(self, u):
        ...
        norm_ = np.sqrt(self.x[:, 3]**2+self.x[:, 3]**2)
        index_ = norm_ > self.v_max
        if any(index_):
            self.x[index_, 2] = self.x[index_, 2]/norm_[index_] *self.v_max
            self.x[index_, 3] = self.x[index_, 3]/norm_[index_] *self.v_max
    ...
    def reset(self):
        ...
        while degree < 2 or min_dist < min_dist_thresh: 
            ...
            length = np.sqrt(np.random.uniform(0, r_max, size=(self.n_agents,)))
    ...
    def render(self, mode='human', targets=None, name=''):
        """
        ...
        if self.fig is None:
            ...
            self.ax.plot(targets[0,:], targets[1,:], 'r+')
